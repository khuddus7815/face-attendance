...
def contact():
    mess._show(title='Contact us', message="Please contact us on :'abdul224601@gmail.com \n 7815969046 ' ")
...
def save_pass():
    ...
    password_file_path = os.path.join("TrainingImageLabel", "psd.txt")

    if os.path.isfile(password_file_path):
        with open(password_file_path, "r") as tf:
            key = tf.read().strip()  # Use strip to remove any whitespace/newline
    else:
        master.destroy()
        ...
        if new_pas is None:
            ...
            return
        with open(password_file_path, "w") as tf:
            tf.write(new_pas)
            mess._show(title='Password Registered', message='New password was registered successfully!!')
            return

    op = old.get().strip()  # Use strip to remove any whitespace
    newp = new.get().strip()
    nnewp = nnew.get().strip()

    if op == key:
        if newp == nnewp:
            with open(password_file_path, "w") as txf:
                txf.write(newp)
...
def TrackImages():
    ...
    try:
        ...
        mess._show(title='Error', message='Face recognition module not found. Please install OpenCV with contrib.')
        return
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if conf < 50:
                ...
                ID = str(ID)[1:-1]
                name = str(aa)[2:-2]
                attendance = [str(ID), '', name, '', str(date), '', str(time_stamp)]
            else:
                ID = 'Unknown'
                name = str(ID)
        ...
        key = cv2.waitKey(1)
        if key == ord('c') and attendance:  # Capture attendance on 'C'
            log_attendance(attendance, col_names)  # Log the attendance
...
def log_attendance(attendance, col_names):
    date = datetime.datetime.now().strftime('%d-%m-%Y')
    attendance_file_path = os.path.join("Attendance", f"Attendance_{date}.csv")
    
    with open(attendance_file_path, 'a+', newline='') as csvFile1:
        writer = csv.writer(csvFile1)
        if os.path.getsize(attendance_file_path) == 0:  # Check if file is empty
            writer.writerow(col_names)
        writer.writerow(attendance)

    # Update TreeView with attendance
...
datef = tk.Label(frame4, text = day+"-"+mont[month]+"-"+year+"  |  ", fg="orange",bg="#262523" ,width=75 ,height=1,font=('times', 18, ' bold '))
datef.pack(fill='both',expand=1)

clock = tk.Label(frame3,fg="orange",bg="#262523" ,width=75 ,height=1,font=('times', 22, ' bold '))
clock.pack(fill='both',expand=1)
tick()

head2 = tk.Label(frame2, text="                       For New Registrations                       ", fg="black",bg="#7D0DC3" ,font=('times', 17, ' bold ') )
head2.grid(row=0,column=0)

head1 = tk.Label(frame1, text="                       For Already Registered                       ", fg="black",bg="#7D0DC3" ,font=('times', 17, ' bold ') )
head1.place(x=0,y=0)
...
takeImg = tk.Button(frame2, text="Take Images", command=TakeImages  ,fg="white"  ,bg="#919A3D"  ,width=34  ,height=1, activebackground = "white" ,font=('times', 15, ' bold '))
takeImg.place(x=30, y=300)
trainImg = tk.Button(frame2, text="Save Profile", command=psw ,fg="white"  ,bg="#919A3D"  ,width=34  ,height=1, activebackground = "white" ,font=('times', 15, ' bold '))
trainImg.place(x=30, y=380)
trackImg = tk.Button(frame1, text="Take Attendance", command=TrackImages  ,fg="black"  ,bg="#3D9A74"  ,width=35  ,height=1, activebackground = "white" ,font=('times', 15, ' bold '))
trackImg.place(x=30,y=50)
quitWindow = tk.Button(frame1, text="Quit", command=window.destroy  ,fg="black"  ,bg="#919A3D"  ,width=35 ,height=1, activebackground = "white" ,font=('times', 15, ' bold '))
